...
export default function WishlistList({ params }: Props) {
  const [countries, setCountries] = useState<CountryFormatted[]>([]);
  const [wishlist, setWishlist] = useState<string[]>([]);

  useEffect(() => {
    // Get storedWishlist from local storage on initial render
  ...
  useEffect(() => {
    ...
    const countryData: Promise<CountryRaw>[] = wishlist.map(
      async (name: string) => {
  ...
  const handleRemoveFromWishlist = (name: string) => {
